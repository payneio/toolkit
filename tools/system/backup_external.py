...
DEFAULT_EXTERNAL_LOCATIONS = ["/media/payne/T9/backups", "/mnt/backup", "/media/backup"]
    ...
    try:
        ...
        if resume and os.path.exists(tar_path):
            ...
            try:
                ...
                logger.warning(
                    f"Existing tar archive appears corrupted, recreating: {tar_path}"
                )
                os.remove(tar_path)
        ...
        cmd = [
            ...
            + os.path.dirname(
                tar_path
            ),  # Exclude the work directory holding our tar file
            "--exclude=lost+found",
            "-C",
            source_parent,
            source_name,
...
def transfer_to_external(source_path, destination_dir, date_str, retention):
    ...
    try:
        ...
        if free_space < required_space:
            ...
            logger.error(
                f"Required: {required_space / 1e9:.2f} GB, Available: {free_space / 1e9:.2f} GB"
            )
            logger.error(
                f"Please free up at least {(required_space - free_space) / 1e9:.2f} GB and try again."
            )
            return False
        ...
        logger.info(
            f"File size: {source_size / 1e9:.2f} GB, Free space: {free_space / 1e9:.2f} GB"
        )
...
def cleanup_external_backups(destination_dir, retention):
    ...
    try:
        ...
        if len(backup_dirs) > retention:
            ...
            for backup_dir in to_delete:
                ...
                try:
                    ...
                    logger.warning(
                        f"Permission error, trying alternative removal method for {dir_path}"
                    )
                    try:
        ...
        logger.info(
            f"Cleanup completed, kept {min(len(backup_dirs), retention)} backups"
        )
    except Exception as e:
...
def create_verification_file(source_files, destination_dir, date_str):
    ...
    try:
        verification_file = os.path.join(
            destination_dir, date_str, "verification.sha256"
        )
...
def main():
    ...
    parser.add_argument(
        "--work-dir", help=f"Work directory for tar files (default: {DEFAULT_WORK_DIR})"
    )
    ...
    if tar_files:
        ...
        if transfer_success:
            ...
            try:
                ...
                logger.info(
                    f"Removed work directory after successful transfer: {work_dir}"
                )
            except Exception as e:
                logger.warning(f"Could not remove work directory {work_dir}: {e}")
        else:
            logger.info(
                f"Preserving work directory due to transfer failure: {work_dir}"
            )
            logger.info("Run with --resume to reuse existing tar files")
    else:
