...
def list_external_backups(destination_dir):
    ...
    if not os.path.exists(destination_dir):
        logger.error(
            f"External destination directory does not exist: {destination_dir}"
        )
        return {}
...
def print_backup_list(backup_sets, source_type):
    ...
    print(
        "\nTo restore, use: backup-restore --source",
        source_type,
        "--date DATE --path PATH",
    )
...
def restore_from_external(
    destination_dir, set_name, date_str, restore_path, target_path, verify_only=False
):
...
def main():
    ...
    parser.add_argument("--set", help="Backup set to restore (e.g., 'data', 'config')")
    parser.add_argument("--path", help="Path within backup to restore")
    parser.add_argument(
        "--target",
    ...
    # Perform restoration
    if args.source == "local":
        success = restore_from_local(
            backup_dir, args.set, args.date, restore_path, target_path
        )
    else:  # external
        success = restore_from_external(
            destination, args.set, args.date, restore_path, target_path
        )
