...
def main():
    ...
    parser.add_argument('--local-only', action='store_true',
                        help="Only run the local backup phase")
    ...
    if args.local_only and args.external_only:
        logger.error("Cannot use --local-only and --external-only together. Choose one.")
        return 1
    ...
    if args.local_only or not args.external_only:
        if not run_backup_collect(args):
            ...
            if not args.external_only:
                logger.warning("Local backup failed, but continuing to external backup phase...")
