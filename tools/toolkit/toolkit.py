...
def get_toolkit_root():
    ...
    if env_path:
        ...
        if not (toolkit_root / "pyproject.toml").exists():
            print(f"Error: TOOLKIT_REPO_PATH is set to '{env_path}' but pyproject.toml not found there.", file=sys.stderr)
            sys.exit(1)
        return toolkit_root
    ...
    if not (toolkit_root / "pyproject.toml").exists():
        ...
        print("Set the TOOLKIT_REPO_PATH environment variable to your toolkit directory:", file=sys.stderr)
...
def create_tool(name, category=None, description="A simple utility"):
    ...
    if not re.match(r'^[a-z][a-z0-9_]*$', name):
        ...
        print("Tool names must start with a letter and contain only lowercase letters, numbers, and underscores.", file=sys.stderr)
        return 1
    ...
    if category:
        ...
        if not tool_dir.exists():
            ...
            print(f"Available categories: {', '.join(sorted([d.name for d in tools_dir.iterdir() if d.is_dir() and not d.name.startswith('_')]))}", file=sys.stderr)
            return 1
    ...
    if script_file.exists():
        print(f"Error: {script_file.relative_to(toolkit_root)} already exists!", file=sys.stderr)
        return 1
    ...
    try:
        ...
        if category_prefix not in category_map:
            ...
            if toolkit_pos != -1:
                ...
                content = content[:toolkit_pos] + new_entry + "\n" + content[toolkit_pos:]
            else:
            ...
            if category_pos != -1:
                ...
                for i, line in enumerate(lines[1:], 1):
                    if line.strip() and not line.strip().startswith('#'):
                        insert_pos = section_start + sum(len(l) + 1 for l in lines[:i+1])
        ...
        print(f"Warning: Could not automatically update pyproject.toml: {e}", file=sys.stderr)
        return False
...
def main():
    ...
    create_parser.add_argument('name', help="Tool name (lowercase letters, numbers, and underscores only)")
    create_parser.add_argument('--category', '-c', help="Add tool to existing category (e.g., document, search)")
    create_parser.add_argument('--description', '-d', default="A simple utility", help="Tool description")
