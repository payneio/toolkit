...
def check_tantivy():
    ...
    if not TANTIVY_AVAILABLE:
        print("Error: Required Tantivy Python bindings not found. Please install them with:")
...
def extract_metadata(file_path: str, extractor_cmd: str) -> Dict[str, Any]:
    """Execute extractor and parse output as YAML or JSON."""
    ...
    try:
        ...
        try:
            ...
            # Try as JSON if YAML parsing fails
            try:
                return json.loads(output)
            except json.JSONDecodeError:
                # If both fail, create basic metadata with content
                return {
...
def save_cache(collection_dir: str, file_path: str, metadata: Dict[str, Any], config: Dict[str, Any]) -> bool:
    ...
    output_format = config.get("output", {}).get("format", "yaml").lower()
    ...
    if output_format == "json":
        ...
        cache_path = os.path.join(cache_dir, f"{cache_filename}.yaml")
        try:
            with open(cache_path, "w") as f:
...
def create_schema():
    ...
    schema_builder.add_text_field("metadata", stored=True)  # For storing additional JSON metadata
...
def get_or_create_index(collection_dir: str) -> tuple:
    ...
    schema_info = {
        "fields": [
            ...
            {"name": "tags", "type": "facet", "stored": False},
    ...
    try:
        ...
        if "tags" in metadata and isinstance(metadata["tags"], list):
            for tag in metadata["tags"]:
                if tag:
                    doc.add_facet("tags", tag.lower().replace(' ', '_'))
        
        # Store all metadata as JSON
        doc.add_text("metadata", json.dumps({
            "path": rel_path,
...
def perform_search(
    ...
    for collection_dir in collections:
        index_dir = os.path.join(collection_dir, INDEX_DIR)
        if not os.path.exists(index_dir) or not os.path.exists(os.path.join(index_dir, "meta.json")):
            continue
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    if os.path.exists(config_path):
        if not args.force:
            sys.stderr.write(f"Error: Collection already exists at {directory}. Use --force to overwrite.\n")
            return 1
...
def cmd_index(args: argparse.Namespace) -> int:
    ...
    directories = [os.path.abspath(d) for d in args.directories] if args.directories else [os.getcwd()]
    ...
    if not collections:
        sys.stderr.write(f"Error: No search collections found in the specified directories.\n")
        return 1
    ...
    for collection_dir in collections:
        ...
        if not config:
            sys.stderr.write(f"Warning: Skipping collection {collection_dir} due to missing or invalid configuration.\n")
            continue
    ...
    print(f"Finished indexing {indexed_count} files across {len(collections)} collections.")
    if failed_count > 0:
...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    if not collections:
        sys.stderr.write("Error: No search collections found. Use --in to specify a collection.\n")
        return 1
...
def main() -> int:
    ...
    # init command
    init_parser = subparsers.add_parser('init', help='Initialize a directory as a search collection')
    init_parser.add_argument('directory', nargs='?', help='Directory to initialize (default: current directory)')
    init_parser.add_argument('--name', help='Name for the collection')
    init_parser.add_argument('--include', nargs='+', help='Include patterns (e.g., "*.pdf" "*.md")')
    init_parser.add_argument('--exclude', nargs='+', help='Exclude patterns (e.g., "*.bak" "draft_*")')
    init_parser.add_argument('--extract', nargs='+', help='Extractor patterns (e.g., "*.pdf=pdf2md {input}")')
    init_parser.add_argument('--force', action='store_true', help='Force overwrite if collection already exists')
    
    # scan command
    scan_parser = subparsers.add_parser('scan', help='Find all search collections in a directory tree')
    scan_parser.add_argument('directory', nargs='?', help='Directory to scan (default: current directory)')
    
    # index command
    index_parser = subparsers.add_parser('index', help='Index all matched files in one or more collections')
    index_parser.add_argument('directories', nargs='*', help='Directories to index (default: current directory)')
    index_parser.add_argument('--verbose', action='store_true', help='Show detailed progress')
    index_parser.add_argument('--no-cache', action='store_true', help='Skip caching extracted metadata')
    
    # query command
    query_parser = subparsers.add_parser('query', help='Search indexed collections using Tantivy')
    query_parser.add_argument('query', help='Search query')
    query_parser.add_argument('--in', dest='in_dir', help='Search in specific collection')
    query_parser.add_argument('--tag', help='Filter by tag')
    query_parser.add_argument('--limit', type=int, default=20, help='Maximum number of results (default: 20)')
    query_parser.add_argument('--verbose', action='store_true', help='Show more detailed results')
