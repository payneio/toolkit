...
def get_connected_devices():
    ...
    try:
        ...
        if devices:
            logger.info(f"Found {len(devices)} connected device(s): {', '.join(devices)}")
        else:
...
def enable_adb_backup(device=None):
    ...
    try:
        ...
        logger.info("2. Navigate to System > Developer options (or About phone > tap Build number 7 times)")
        logger.info("3. In Developer options, find and enable 'USB debugging' if not already enabled")
        logger.info("4. For app data backup, also enable 'USB debugging (Security settings)'")
        ...
        logger.error("Please ensure your device is connected and USB debugging is authorized.")
        return False
        ...
        for file_path in filtered_files:
            ...
            try:
                ...
                if success_count % 10 == 0:
                    logger.info(f"Pulled {success_count}/{len(filtered_files)} files...")
            except subprocess.CalledProcessError as e:
                logger.warning(f"Failed to pull {file_path}: {e}")
        
        logger.info(f"Successfully pulled {success_count}/{len(filtered_files)} files to {dest_path}")
...
def backup_app_data(app_id, dest_path, device=None, compression_level=9):
    ...
    try:
        ...
        while process.poll() is None:
            logger.info("Backup in progress... Please confirm on your device if prompted")
            time.sleep(5)
    ...
    for item in os.listdir(backup_dir):
        ...
        try:
            ...
            for subdir in os.listdir(item_path):
                ...
                if os.path.isdir(subdir_path) and re.match(r'^\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}$', subdir):
            ...
            logger.info(f"Cleanup completed in {item_path}, kept {min(len(dated_dirs), retention)} backups")
        except Exception as e:
...
def backup_rcs(dest_path, device=None, format="json"):
    ...
    try:
        ...
        if "Error" in result.stderr or not result.stdout.strip():
            logger.warning("Standard RCS query failed, attempting to backup via Google Messages app data")
...
def backup_contacts(dest_path, device=None, format="vcf"):
    ...
    try:
        if format.lower() == "vcf":
            ...
            temp_vcf = "/sdcard/contacts_backup.vcf"
            export_cmd = adb_cmd + [
                "shell", 
                f"am start -a android.intent.action.MAIN -n com.android.contacts/.activities.PeopleActivity"
            ]
            ...
            if query_result.returncode == 0 and query_result.stdout.strip():
                ...
                for line in contacts_data.splitlines():
                    if not line.strip():
                        if current_contact:
                            ...
                            if "data1" in current_contact and "data2" in current_contact and current_contact.get("data2") == "1":
                                # Phone number
                                vcf_content += f"TEL:{current_contact['data1']}\n"
                            if "data1" in current_contact and "data2" in current_contact and current_contact.get("data2") == "2":
                                # Email
                ...
                logger.warning("Direct contacts query failed, attempting to backup via Contacts app data")
...
def main():
    ...
    parser.add_argument('--photos-only', action='store_true',
                        help="Only backup photos")
    parser.add_argument('--videos-only', action='store_true',
                        help="Only backup videos")
    parser.add_argument('--documents-only', action='store_true',
                        help="Only backup documents")
    parser.add_argument('--sms-only', action='store_true',
                        help="Only backup SMS messages")
    parser.add_argument('--rcs-only', action='store_true',
                        help="Only backup RCS messages")
    parser.add_argument('--contacts-only', action='store_true',
                        help="Only backup contacts")
    parser.add_argument('--call-logs-only', action='store_true',
                        help="Only backup call logs")
    parser.add_argument('--app-data', 
                        help="Only backup specific app data (e.g., 'com.whatsapp')")
    ...
    if not check_adb():
        logger.error("ADB is required but not found. Please install Android Debug Bridge.")
        return 1
    ...
    devices = get_connected_devices()
    if not devices:
        logger.error("No Android devices connected. Please connect a device and try again.")
        return 1
    ...
    if args.device:
        if args.device in devices:
            ...
            logger.error(f"Specified device {args.device} not found in connected devices.")
            return 1
    elif len(devices) > 1:
        logger.warning("Multiple devices connected. Please specify a device with --device.")
    ...
    if args.photos_only:
        ...
        types_to_backup = [btype["name"] for btype in config["backup_types"] if btype.get("enabled", True)]
        ...
        if backup_type["name"] == "app_data":
            ...
            for app_id in backup_type["apps"]:
                ...
                if not backup_app_data(
                    ...
                    app_backup_dir, 
                    device, 
                    config.get("compression_level", 9)
                ):
            ...
            type_backup_dir = os.path.join(backup_base_dir, backup_type["name"], date_str)
            ...
            for source_path in backup_paths:
                ...
                if file_extensions:
                    logger.info(f"Backing up {backup_type['name']} from {source_path}...")
                    ...
                    for ext in file_extensions:
                        ...
                        find_cmd = adb_cmd + ["shell", f"find {source_path} -type f -iname '*{ext}'"]
                        find_result = subprocess.run(find_cmd, capture_output=True, text=True, check=False)
                        
                        if find_result.returncode == 0 and find_result.stdout.strip():
                            files = [f.strip() for f in find_result.stdout.splitlines() if f.strip()]
                            logger.info(f"Found {len(files)} {ext} files in {source_path}")
                    ...
                    if not pull_files(source_path, type_backup_dir, device, path_exclusions):
