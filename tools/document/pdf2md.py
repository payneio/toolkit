...
def main():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            content = re.sub(r'(?m)^\s*\d+\s+The Social Issues Research Centre.*$', '', content)
            ...
            result = subprocess.run(
                ["pandoc", temp_txt, "-f", "markdown+simple_tables+table_captions+yaml_metadata_block", 
                 "-t", "markdown_github", "--wrap=none", "--standalone"],
                check=True,
                capture_output=True,
                text=True
            )
            ...
            # Update the result with cleaned markdown
            result_cleaned = type('obj', (object,), {'stdout': markdown})
        
        # Write markdown to file
        with open(output_file, 'w') as f:
            f.write(result_cleaned.stdout)
        
        # Also output to stdout
        print(result_cleaned.stdout)
        ...
        if "pdftohtml" in str(e):
            print("Error: pdftohtml is not installed. Please install it with 'apt install poppler-utils'", file=sys.stderr)
        else:
